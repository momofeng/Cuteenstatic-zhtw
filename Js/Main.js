...
var CuteenFunc = {
	...
	startSearch: function (item) {
		...
		$(item).blur();
		if (!c || c == '') {
			$(item).attr('placeholder', '你還沒有輸入任何信息');
			return;
		}
			...
			((function () {
				...
				function loop() {
					...
					if (new Date() - startTime > timeLimit) {
						...
						open = true; window.stop(); alert('大佬別再扒了！趕緊買一份吧ヽ(￣ω￣(￣ω￣〃)ゝ'); document.body.innerHTML = "";
					} else {
						open = false;
					}
				}
...
function AjaxComment() {
	$('#comment-form').submit(function () {
		...
		$.ajax({
			...
			error: function () {
				iziToast.error({ title: '評論失敗', message: '參數提交錯誤，請刷新後重試', position: 'topRight', })
				$("#comment-submit-button").removeClass('lv').addClass('hong').text('提交失敗');
				return false;
			},
			success: function (data) {
				$("#comment-submit-button").removeAttr("disabled", "disabled").text('提交評論');
				var error = /<title>Error<\/title>/;
				if (error.test(data)) {
					var text = data.match(/<div(.*?)>(.*?)<\/div>/i);
					var str = '回傳參數錯誤，請刷新後重試';
					if (text != null) str = text[2];
					iziToast.error({ title: '評論失敗', message: str, position: 'topRight' })
				} else {
					//評論框復位（清空文本，刷新高度）
					$('#textarea').val('');
					$('#textarea').css('height', '');
					...
					if ($('.All_Pagination .prev').length && !parent) {
						var dd = $(".prev a").attr("href"); //獲取上頁地址
						$(".prev a").attr("href", ""); //將地址清空
						dd = dd.replace(/comment-page-(.*?)#comments/, "comment-page-1#comments"); //將獲取的地址頁碼改為1
						$(".prev a").attr("href", dd); //將地址放回去
						$('.prev a').get(0).click(); //點擊這個超鏈接
					} else {
						new_id = '';
					} //判斷當前評論列表是否在第一頁,並且只會在母評論時候才會生效 */
					new_id = $(".comment-list", data).html().match(/id=\"?comment-\d+/g).join().match(/\d+/g).sort(function (a, b) {
						return a - b
					}).pop();
					$('#commentcontent').html($('#commentcontent', data).html()); //更新評論列表
					//下面是重載函數以及評論成功提示
					iziToast.success({ title: '評論成功', message: '您的留言已收到', position: 'topRight' })
					//跳轉到新ID
					if (new_id) {
